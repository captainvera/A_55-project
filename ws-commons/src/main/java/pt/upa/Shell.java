...
public abstract class Shell {
  ...
  public Shell(String n, Writer w, boolean flush) {
    ...
    new Command(this, "run", "run a class method") {
      public void execute(String[] args) {
	      try {
	        if (args.length > 0) shell().run(args[0], Arrays.copyOfRange(args, 1, args.length));
  ...
  public void execute() throws Exception {
    ...
    String str, prompt = "FrontEnd > "; // System.getenv().get("PS1");

    //if (prompt == null) prompt = "6P@MLG$420 > ";
    System.out.println("--- Welcome to Upa Service FrontEnd ('quit' to leave)");
